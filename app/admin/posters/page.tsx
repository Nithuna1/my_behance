...
export default function PostersAdmin(){
  ...
  const submit=async()=>{
    ...
    if(editId){
      await fetch("/api/posters",{method:"PUT",body:JSON.stringify({id:editId,...form})});
      setEditId(null);
    }else{
      await fetch("/api/posters",{method:"POST",body:JSON.stringify(form)});
    }
  ...
  const del=async(id:string)=>{
    await fetch("/api/posters",{method:"DELETE",body:JSON.stringify({id})});
    load();
  };
